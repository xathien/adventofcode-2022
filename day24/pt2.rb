...
second_start = first_time
while all_grids[second_start % all_grids.size].key?([target_x, target_y])
  second_start += 1
end

start_node = [target_x, target_y, second_start]
came_from = {}
g_scores = Hash.new { |h, k| h[k] = Float::INFINITY }
g_scores[start_node] = 1
...
until queue.empty?
  ...
  moves.each { |dx, dy|
    ...
    if next_g_score < g_scores[target]
      came_from[target] = current
      g_scores[target] = next_g_score
      distance_heuristic = next_g_score + next_x + next_y
      queue.push(target, distance_heuristic)
    end
  }
end

third_start = second_time
while all_grids[third_start % all_grids.size].key?([target_x, target_y])
  third_start += 1
end

start_node = [0, 0, third_start]
g_scores = Hash.new { |h, k| h[k] = Float::INFINITY }
g_scores[start_node] = 1
